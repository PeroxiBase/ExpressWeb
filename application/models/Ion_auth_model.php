...
class Ion_auth_model extends CI_Model
{
	...
	public function activate($id, $code = false)
	{
		...
		if ($code !== FALSE)
		{
			$query = $this->db->select("$this->identity_column,email")
			...
			$username = $result["$this->identity_column"];
			if ($query->num_rows() !== 1)
			{
			...
			$extra_data = array( 'Subject' => 'activate_successful','config_item' => 'email_activate_user', 'trigger_item' => '',
			    'trigger_result' =>'' ,'trigger_message' => '','set_message' => 'Account activated',
			    'message' => " Your account $username have been activated on ExpressWeb",
			    'data' =>array('identity' => $username,'id' => $id,'activation' => 1));
		...
		else
		{
			...
			$profile  = $this->where('id', $id)->users()->row(); 
			$to_email = $profile->email;
			
			$extra_data = array( 'Subject' => 'activate_successful','config_item' => 'email_activate_user', 'trigger_item' => '',
			    'trigger_result' =>'' ,'trigger_message' => '','set_message' => 'Account activated',
			    'message' => " Your account $profile->username have been activated on ExpressWeb",
			    'data' =>array('identity' => $profile->username,'id' => $id,'activation' => 1));
		...
		if ($return)
		{
			...
			$this->send_email($to_email,$extra_data);
			//print "send mail";
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				// BS check group users
				$groups= $this->get_users_groups($user->id);
				$user_groups=array();
				foreach($groups->result() as $row)
				{
				    array_push($user_groups,$row->name);
				}
	...
	**/
	...
	public function set_session($user)
	{
		...
		// BS
		if(in_array('Admin',$user->groups)) $session_data['isAdmin'] ='Admin';
		if(in_array('Editor',$user->groups)) $session_data['isAdmin'] ='Editor';
		if(in_array('Redactor',$user->groups)) $session_data['isAdmin'] ='Redactor';
	...
	**/
		...
		function manage_project()
	{
		...
		if ($delete_projects = $this->input->post('delete_project'))
		{
			...
			{
				// Note: As the 'delete_project' input is a checkbox, it will only be present in the $_POST data if it has been checked,
				// therefore we don't need to check the submitted value.
				$this->flexi_auth->delete_custom_user_data('user_project', $project_id);
			}
		}
		#var_dump($this->input->post());exit;
		/**
		* Define Working  projects.
		* change uprj_working flag to 1 to selected project. Disable other users project
		*/
		if ($working_projects = $this->input->post('working_project'))
		{
		 // Split working_project POST:  id|Project Name
		  list($id_project,$working_project)= explode('|',$working_projects);
		  // Create flexi_auth session working_project
		  #working_project=preg_replace('-\\\.\*
		  /*
		  $str = preg_replace('/[^A-Za-z0-9\. -]/', '', $str);
		  $str = preg_replace('/\\s+/', '-', $str);
		  */
		  $working_project = trim($working_project);
		  $this->auth->session_data[$this->auth->session_name['working_project']] = $working_project;
			$this->session->set_userdata(array($this->auth->session_name['name'] => $this->auth->session_data));
			$this->auth->session_data[$this->auth->session_name['working_project_id']] = $id_project;
			$this->session->set_userdata(array($this->auth->session_name['name'] => $this->auth->session_data));
		  // Get Current User_Id
		  $user_id = $this->flexi_auth->get_user_id();
		  // Define working projet 
		  $set_working_prj="UPDATE `Genes`.`user_project` SET `uprj_working` = '1' WHERE `user_project`.`uprj_id` =$id_project";
		  $response = $this->db->query($set_working_prj);
		  // Disabel other projects
		  $disable_other_prj="UPDATE `Genes`.`user_project` SET `uprj_working` = '0' WHERE `user_project`.`uprj_uacc_fk`=$user_id AND `user_project`.`uprj_id` !=$id_project";
		  $do_disable=$this->db->query($disable_other_prj);
		  # $response = $this->flexi_auth->update_custom_user_data('user_project', $id_project, $project_data);
		  // Create workspace in assets/users/$username/$working_project
		  $this->genes_functions->working_space();
		  $this->session->set_flashdata('message', $this->flexi_auth->get_messages());
		#	print "set_working_prj  $set_working_prj disable_other_prj  $disable_other_prj<br />";exit;
			// Redirect user.
			($response) ? redirect('auth_public/manage_project') : redirect('auth_public/update_project');
		}
	...
	function update_project($project_id)
	{
		...
		$validation_rules = array(
			array('field' => 'update_project_name', 'label' => 'Project Name', 'rules' => 'required'),
			array('field' => 'update_creator', 'label' => 'Creator', 'rules' => 'required'),
			array('field' => 'update_project_comment', 'label' => 'Comment', 'rules' => 'required'),
			array('field' => 'update_date_start', 'label' => 'Date Start ', 'rules' => 'required'),
			array('field' => 'update_visible', 'label' => 'Visible ', 'rules' => 'boolean'),
			array('field' => 'update_shared', 'label' => 'Shared ', 'rules' => 'boolean'),
		);
		...
		if ($this->form_validation->run())
		{
			...
			$project_data = array(
				'uprj_uacc_fk' => $this->input->post('update_creator'),
				'uprj_project_name' => $this->input->post('update_project_name'),
				'uprj_project_comment' => $this->input->post('update_project_comment'),
				'uprj_project_visibility' => $this->input->post('update_visible'),
				'uprj_project_shared' => $this->input->post('update_shared'),
				'uprj_project_date_start' => $this->input->post('update_date_start'),
				'uprj_project_date_end' => $this->input->post('update_date_end'),
				'uprj_project_shared' => $this->input->post('update_shared'),
			);		
		#	print "<br /> Visible ".$this->input->post('update_visible')." <br />";
			// For added flexibility, to identify the table and row to update, you can either submit the table name and row id via the 
			// first 2 function arguments, or alternatively, submit the primary column name and row id value via the '$project_data' array.
			...
			$response = $this->flexi_auth->update_custom_user_data('user_project', $project_id, $project_data);
			 ...
			 * 		// ... etc ... // 
			 *	);
			 * 	$response = $this->flexi_auth->update_custom_user_data(FALSE, FALSE, $project_data);
			*/
        ...
        public function send_email($to_email,$config_array)
        {
            $subject = $config_array['Subject'];
            $config_item = $config_array['config_item'];
            $trigger_item = $config_array['trigger_item'];
            $trigger_result = $config_array['trigger_result'];
            $trigger_message = $config_array['trigger_message'];
            $set_message = $config_array['set_message'];
            $message = $config_array['message'];
            $data = $config_array['data']; 
            
            $message = $this->load->view($this->config->item('email_templates', 'ion_auth').$this->config->item($config_item, 'ion_auth'), $data, true);

            $this->email->clear();
            $this->email->from($this->config->item('admin_email', 'ion_auth'), $this->config->item('site_title', 'ion_auth'));
            $this->email->to($to_email);
            $this->email->subject($this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line($subject));
            $this->email->message($message);

            if ($this->email->send() == TRUE)
            {
                  #  $this->ion_auth_model->trigger_events(array($trigger_item, $trigger_result, $trigger_message ));
